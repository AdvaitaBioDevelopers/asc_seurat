...
suppressMessages(require(BiocManager))
...
suppressMessages(require(BiocVersion))
#
# dynverse packages
suppressMessages( require(dynmethods) )
...
function(input, output, session) {
    ...
    single_cell_data_reac <- eventReactive( input$load_10X, {
        ...
        sing_cell_data.data <- Seurat::Read10X(file.path(getwd(),files[files$Length==0,][,1]))
        # unlink(file.path(getwd(),files[files$Length==0,][,1]), recursive = T)
        unlink(file.path(getwd(),c(files[files$Length==0,][,1])), recursive = T)
...
# Send the data to iPG
    ...
    samples_list_integration <- reactive({
        ...
        req(input$multi_upload)
        unzip(input$multi_upload$datapath)
        files <- unzip(input$multi_upload$datapath, list = TRUE)
        # sing_cell_data.data <- Seurat::Read10X(file.path(getwd(),files[files$Length==0,][,1]))
        # unlink(file.path(getwd(),files[files$Length==0,][,1]), recursive = T)
        # unlink(file.path(getwd(),c(files[files$Length==0,][,1])), recursive = T)
    ...
    output$find_markers_or_DE_tab2_cluster1_ui <- renderUI ({
        clusters <- req( clusters_single_cell_data_reso_umap_tab2() )
        shinyWidgets::pickerInput(
            inputId = "find_markers_or_DE_tab2_cluster1",
            # label = "Select the cluster of interest",
            label = "Choose the cluster/cell type",
            choices = sort(clusters),
            multiple = FALSE,
            options = list(`actions-box` = TRUE)
        )
    ...
    output$find_markers_or_DE_tab2_treat1_ui <- renderUI({
        ...
        shinyWidgets::pickerInput(
            ...
            label = "Select the Experimental group",
    ...
    output$find_markers_or_DE_tab2_cluster2_ui <- renderUI ({
        clusters <- req( clusters_single_cell_data_reso_umap_tab2() )
        clusters <- clusters[ !clusters == "find_markers_or_DE_tab2_cluster1"]
        shinyWidgets::pickerInput(
            inputId = "find_markers_or_DE_tab2_cluster2",
            # label = "Select the cluster of interest",
            label = "Choose the cluster/cell type",
            choices = sort(clusters),
            multiple = FALSE,
...
output$find_markers_or_DE_tab2_treat2_ui  <- renderUI({
    treat <- req( treat_single_cell_data_reso_umap_tab2() )
    treat <- treat[!treat == input$find_markers_or_DE_tab2_treat1]
    ...
    markers_tab2 <- eventReactive( input$run_ident_markers_tab2, {
        ...
        if (input$find_markers_or_DE_tab2 == 0 ) {
            ...
            markers_tab2 <- FindMarkers(sc_data,
                                        ident.1 = paste(req(input$find_markers_or_DE_tab2_cluster1),
                                                        req(input$find_markers_or_DE_tab2_treat1),
                                                        sep = "_"),
                                        # ident.2 = paste(req(input$find_markers_or_DE_tab2_cluster),
                                        ident.2 = paste(req(input$find_markers_or_DE_tab2_cluster2),
            ...
            markers_tab2$comparison <- paste("Cluster", input$find_markers_or_DE_tab2_cluster1,
                                             input$find_markers_or_DE_tab2_treat1,
                                             "vs",
                                             input$find_markers_or_DE_tab2_cluster2,
        ...
        filename = function() {
            ...
            if (input$find_markers_or_DE_tab2 == 0 ) {
                ...
                paste("list_of_markers_int.dataset_DEGs_",
                      input$find_markers_or_DE_tab2_cluster1,
                      input$find_markers_or_DE_tab2_treat1,
                      "_vs_",
                      input$find_markers_or_DE_tab2_treat2,
                      "_cluster_",
